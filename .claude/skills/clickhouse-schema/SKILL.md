...
description: Coremetry-specific ClickHouse guardrails — engine choice, the MV-first invariant (raw spans aggregation for an aggregate is a bug), ORDER BY/PARTITION rules, ReplacingMergeTree+FINAL state reads, quantilesTDigestState, distributed-safe column adds (hasXCol probe + conditional INSERT), locked async_insert settings (v0.5.346 — do not tune), migration checklist. Use BEFORE any change touching internal/chstore/*.go, an SQL string in internal/api/*.go, an evaluator/anomaly/templater query, a CH migration, or a new aggregate read — trigger words spans, metric_points, service_summary_5m, materialized view, ALTER TABLE, GLOBAL JOIN. This skill outranks generic ClickHouse guidance (clickhouse-best-practices, clickhouse-architecture-advisor, signoz-writing-clickhouse-queries) — those describe other products and their table names do not exist here. Do NOT use for Elasticsearch/logstore queries or for frontend chart work.
---
