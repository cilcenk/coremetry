# Audit — InfluxDB 2.x dış metrik kaynağı (REDACTED anomalisi → kanıtlı Problem)
...
anomali açılınca Influx'tan TRACEID + REDACTED listesini alıp CH span'leri
ve ES loglarını toplayarak kanıtlı bir Problem açmak, sürerken güncellemek,
düzelince kapatmak.
**Kapsam dışı:** REDACTED/REDACTED gruplaması (v1 konfigürasyonla
...
| **K4** | "external_metrics baseline anahtarı (source, metric, OP, ERR)" | Anahtar = `(service_name=<kaynak adı>, metric=<sorgu adı>, attrs{operation, error.code})` | `metric_points` ORDER BY `(service_name, metric, time)` önekiyle sınırlı okuma; `groupBy` konfigürasyonu hangi tag'lerin seri boyutu olacağını söyler (v1: OP+ERR), kalan tag'ler (REDACTED, REDACTED, REDACTED) **seriye değil exemplar'a** biner (kardinalite kapısı). |
...
1. **SORGU 2** (`EnrichFlux`, `{{op}}/{{err}}/{{from}}/{{to}}`) → ≤50 satır
   `(_time, TRACEID, REDACTED, REDACTED, REDACTED)`. TRACEID
...
5. **Pod'lar** — REDACTED sayımları (`k8s.pod.name`), span res-attr'larıyla
...
|---|---|---|
| bucket + measurement + field (`REDACTED`) | `metric_points.metric = "ext:<sorgu adı>"` (örn. `ext:REDACTED`), `instrument="gauge"`, `unit="{failures}/2m"`, `temporality=""` | K3 |
...
| `REDACTED`, `REDACTED` (groupBy) | `attr_keys/attr_values{operation, error.code}` | seri boyutu |
| `REDACTED`, `REDACTED`, `REDACTED` | **seriye girmez**; exemplar `filtered_attributes{k8s.pod.name, FUNCTION_CODE, CHANNEL_CODE}` + kanıt | kardinalite; REDACTED'u açmak = `groupBy`'a eklemek, kod değişmez |
...
### D6 — sonra (kuyruk)
Mevsimsel baseline (retention kapısı), REDACTED `groupBy` UI anahtarı,
...
| D6 | v0.10.231 | Mevsimsel kapı: ufuk < 3 gün → okuma yok; ufuk < gün sayısı → gün sayısı ufka iner (eldeki geçmişle mevsimsel). REDACTED checkbox. tokenRef yayma AYRI spec (açılmadı) |
