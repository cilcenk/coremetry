...
| A5 🟡 | FieldStats zarfı `esSearchEnvelope`'u gömmüyor; bekçi testi yalnız `var raw struct` + tek dosya tarıyor | `elasticsearch.go:1123`, `es_envelope_test.go:135-143` | embed + bekçiyi `es_*.go` ve `raw\|decoded`'a genişlet | ~20 dk |
...
| A7 🟡 | `/logs/context` iki Search ardışık (yorum "parallel"), her yarı `track_total_hits:10000` + tam `_source` | `api_logs.go:771,869,876` | errgroup + CountMode=false | ~30 dk |
