...
| A3 🟠 | **GEMİDE v0.10.428** — `buildQuery` her şeyi `must`'a koyuyor (zaman, service, term, exists, severity) — filter context yok | `elasticsearch.go:2447,2745` | serbest metin dışını `bool.filter`'a; altın-gövde testi | ~1 saat |
...
| A6 🟡 | Pencere ms hassasiyetli → `request_cache`/`serveCached` paylaşılmıyor | `api_logs.go:396`, `frontend/src/lib/utils.ts:21-24`; doğru emsal `es_trace_context.go:47` | handler'da `to`'yu 10 dk'ya truncate | ~1 saat | **V1 GEMİDE v0.10.442** — yalnız serveCached ANAHTARI 10 dk sınıra oturur (canlı pencere, süre korunur; mutlak pencere aynen; desenler ucunda sunucu varsayılan penceresi de anahtara girer); sorgu ms hassasiyetiyle aynen koşar. **V2 GEMİDE v0.10.446 (operatör kararı: "uç TTL'i kadar")** — canlı pencere ucun TTL'ine (15/30/60 sn) oturur ve sorgu da o pencereyle koşar → ES request_cache isabet eder; uzunluk korunur, sağ uç ≤ TTL geriye kayar; mutlak pencere aynen |
...
| # | Bulgu | Kanıt | Fix | Tahmin |
|---|---|---|---|---|
| C1 | Problem panelinde log kanıtı toplanıyor ama çizilmiyor (`templates?: unknown[]`) | `types.ts:914`, `RootCausePanel.tsx:282` | "Log kanıtı" Section + `logsHref(q: PatternSearchQuery)` | ~2 saat | **GEMİDE v0.10.452** — Problem detayında "Log kanıtı" (ProblemLogEvidence): servis kapısı içinde, varsayılan kapalı, açılınca TEK /api/logs/patterns (sample=500 → 1 ES sayfası), pencere problemWindowNs, ERROR+, degraded→partial→boş, total "≥" dürüstlüğü, satır pivotu /logs?panel=patterns |
...
| C3 | `familyLogs` kanıtı filo sıralı + ömür-boyu sayım (Service alanı geçilmiyor, TotalCount kümülatif) | `anomaly/investigation.go:152,408`, `log_templates.go:17` | `Service` geç + pencere sayımı (`GroupBySignature`) | ~yarım gün | **GEMİDE v0.10.452** — familyLogs `Service` süzgeciyle (kalıcı log_templates, CH; ES'e gidilmez — operatör kısıtı); sayım ömür boyu kalır ve RootCausePanel "Log şablonları (bu servis)" alt başlığı bunu söyler; pencere sayımı bilinçli yapılmadı |
| C4 | Desenler örneklemesi en-yeni-2000; panel kapsanan alt pencereyi söylemiyor | `patterns.go:75`, `LogPatternsPanel.tsx:122` | `coveredFromNs/ToNs` + altbilgi | ~1 saat | **GEMİDE v0.10.441** — `coveredFromNs/coveredToNs` (okunan satırların min/max), panel altbilgisi "kapsanan: HH:MM:SS–HH:MM:SS (N dk, en yeni uç)" yalnız sampled ≥ cap iken (truncated değil), degraded gövde aynı şekil |
