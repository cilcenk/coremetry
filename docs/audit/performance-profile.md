...
| ~~S3~~ | ~~`metric_points` … `metric` skip index~~ **ÇÜRÜTÜLDÜ (v0.10.261, lokal ölçüm):** yalnız-metric filtresi PK'de zaten 437→278 granül budadı; `set(0) GRANULARITY 4` index eklenip materialize edilince Skip 278/278 (sıfır ek budama), read_rows aynı (2.05M), medyan **1592 → 3901 ms** (index analizi bedeli). Sebep: ORDER BY (service_name, metric, time) içinde metric her servis aralığında yaşar, PK'nin geçirdiği granüller zaten o metriği içerir; index ancak NADİR metriklerde budar. Migration YAZILMADI; alternatif PROJECTION (metric, time) = disk ×2, ayrı karar | — | — | — |
...
| 5 | **`metric_points` okuma şekli** — S3 index ÇÜRÜTÜLDÜ (ölçüm yukarıda, migration yok); CDV-5 auto-step → rollup planı **GEMİDE v0.10.261** | M-L | M | `internal/chstore/store.go`, `migrations/0014_metric_points_metric_index.sql`, `internal/chstore/metric_rollup_read.go`, `metricquery.go` |
| 6 | **Grafik hacmi** — CDV-1 sparklines ≤120 slot + CDV-2 açık adım nokta bütçesi **GEMİDE v0.10.262**; CDV-3 (groupBy top-N sunucuda) ERTELENDİ: groupBy SQL'i test-pinli, top-N alt sorgu grup arg'larını çiftler; Explore eski yolda spanMetricTopN var — ayrı dilim | M | M | `internal/api/api.go` (sparklines/queryMetric handler'ları → kendi dosyalarına), `internal/chstore/metricquery.go`, `frontend/src/lib/queries/services.ts` |
| 7 | **Şema hijyeni** — S2 + S5 **GEMİDE v0.10.263 şema değişikliği OLMADAN**: retention enforcer MV depolarına genişledi (`retention_mv.go`: spanmetrics_1s 1 g, _10s 2 g, trace MV'leri retention.spans+1 g; `.inner_id.<uuid>` küme genelinde, ON CLUSTER DROP PARTITION). Saatlik partition (≤48 s yerine ≤1-2 s) istenirse migration — ayrı karar. S1/S8 şema drift raporu ERTELENDİ | M | M | `internal/chstore/store.go`, `migrations/0015_*.sql`, `internal/chstore/cluster.go`, `internal/api/admin_schema_drift.go` (yeni), `frontend/src/pages/system/*` |
