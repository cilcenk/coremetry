...
| C3 | TTL (30 s) = cacheBucket grid (30 s) → SWR/STALE penceresi kayan sorgularda hiç kullanılmıyor | `cache.go:183-190` | M | TTL'i grid'in 2-3 katı yap ya da grid'i TTL'in yarısı | S |
...
| 2 | **Cache anahtarı basamağı + ısıtma anahtarı** (C1, C2, C3, C4): 14 uçta `cacheBucket`, `refresh=1` anahtardan düş, TTL/grid oranı, warm anahtarı tek kaynak | M-L | M | `internal/api/api.go` (key builder'lar), `internal/api/cache.go`, `internal/api/cache_key_test.go` |
| 3 | **Ayar yenileyici tekilleştirme** (P-set): bir toplu okuma / 30 s, abonelere dağıtım | M | S | `internal/chstore/settings.go` (`GetSettings`), `internal/tempo/client.go` (şablon), `main.go` kablolama, `internal/*/settings.go` tüketiciler |
| 4 | **Inbox/problems okuma yolu** (F3, F4, F-list): blast-radius toplu ucu, SSE coalescing, `ListProblems` zaman sınırı + çözülmüşlere TTL | M-L | M | `internal/api/problems*.go` (yeni `blast_radius_routes.go`), `frontend/src/features/anomalies/ProblemsSection.tsx`, `frontend/src/lib/queries/eventStream.ts`, `internal/chstore/problem.go`, `store.go` (TTL) |
...
| 8 | **Sorgu etiketleme** (P-comment) — diğer maddelerin ölçüm ön koşulu: `log_comment=route:<pattern>`/`worker:<name>` | M (denetlenebilirlik) | S | `internal/chstore/traced_conn.go` (ctx'ten etiket), `internal/api/cache.go` (route → ctx), `.claude/skills/perf-triage/SKILL.md` (bilinen boşluk kapanır) |
