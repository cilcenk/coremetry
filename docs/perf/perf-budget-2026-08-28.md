...
| P7 | arka plan `problems FINAL` envanteri (evaluator/monitor/anomaly/watcher dedup) | 16 tam-satır tarama/dk (12:52–13:02) | **≤ 8/dk** | ≤ 6/dk | **v0.10.156–158 GEMİDE, ÖLÇÜLDÜ (2026-08-29, minikube chc-0, query_log şekle göre sayım):** 156 = `OpenProblemsSnapshot` 5 s memo + arka plan çağrıcılar snapshot'tan; inceleme D1–D3 (kopya dönen ByKey/ByID, kilitsiz Invalidate, süpürme/tick başına tek snapshot). 156 sonrası: per-rule `FindOpenProblem` 24→0, arka plan `ListProblems` 75→14 tasarlandığı gibi — ama snapshot 47→**93**/10 dk: D3 hoist'i monitor tick'inde (5 s) KOŞULSUZ okuyordu ve memo TTL'i (5 s, operatör kararı) tick aralığına eşit olunca her tick ıskaladı. 158 = `chstore.LazySnapshot` (ilk ihtiyaçta bir kez): 10 dk pencerede tam-satır tarama **160 → 101 (−37 %)**: snapshot 55 (evaluator tick kadansı ≈ 5.5/dk), API `ListProblems` 15, tek-id poll 21 (açık bir problem sekmesi), 0 per-rule. CPU-s 18.1 → 13.7 (−25 %, gürültülü: cpuwait p50 150–416 ms, lokal CH demo ingest altında hiç sakin değil — [[feedback-perf-benchmark-discipline]]). Kalan 5.5/dk = tick başına bir tarama; ≤ 3/dk için TTL ≥ evaluator aralığı (10 s) ya da işçilerin tek tick'e hizalanması gerekir — operatör 5 s'i seçti, yeniden açılmadı. |
