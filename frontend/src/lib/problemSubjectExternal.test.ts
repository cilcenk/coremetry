...
describe('external özne', () => {
  it("kind='external' → external, link yok", () => {
    expect(subjectKind('ext:REDACTED/OP1/E1', 'external')).toBe('external');
    expect(subjectIsLinkable('ext:REDACTED/OP1/E1', 'external')).toBe(false);
  });
  it('kind yokken ext: öneki yeter', () => {
    expect(subjectKind('ext:REDACTED/OP1/E1')).toBe('external');
    expect(subjectKind('ext:REDACTED/OP1/E1', '')).toBe('external');
  });
  it('etiket: kaynak · grup değerleri', () => {
    expect(subjectLabel('ext:REDACTED/OP1/E1')).toBe('REDACTED · OP1 · E1');
    expect(subjectLabel('ext:REDACTED')).toBe('REDACTED');
  });
  it('başlık dış kaynağı açıklar', () => {
    expect(subjectTitle('ext:REDACTED/OP1/E1')).toMatch(/dış|Influx/i);
  });
