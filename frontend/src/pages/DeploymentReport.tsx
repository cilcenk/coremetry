...
export default function DeploymentReportPage() {
  ...
  // v0.10.79 uyarlaması — datetime-local YASAK (v0.9.879, clockFormat
  // kapısı): native widget en-US tarayıcıda AM/PM çizer ve `lang` güvenilir
  // bir override değil. MaintenanceTab emsali: elle çizilmiş metin girdisi,
  // global zaman seçicinin AYNI "YYYY-MM-DD HH:mm:ss" grameri ve AYNI
  // parser'ı. PR bu kuraldan ÖNCE yazılmıştı.
  const [pendingLocal, setPendingLocal] = useState(() => {
    if (!sinceParam) return '';
    return formatDateTime(Number(sinceParam) / 1_000_000);
  });
  const [pendingError, setPendingError] = useState<string | null>(null);

  const generate = () => {
    if (!pendingLocal) return;
    const ms = parseDateTime(pendingLocal.trim());
    if (ms === null) {
      setPendingError('Geçersiz tarih — beklenen biçim YYYY-MM-DD HH:mm:ss');
      return;
    }
    ...
    <>
      ...
      <PageShell>
        ...
        <div className="controls" style={{ marginBottom: 16, alignItems: 'flex-end', gap: 12 }}>
          <Field
            label="Deployment succeeded at (YYYY-MM-DD HH:mm:ss)"
            placeholder={formatDateTime(Date.now())}
            value={pendingLocal}
            onChange={e => { setPendingLocal(e.target.value); setPendingError(null); }}
          />
          <Button variant="primary" size="sm" onClick={generate} disabled={!pendingLocal}>
            Generate report
          </Button>
          {pendingError && (
            <span style={{ color: 'var(--err)', fontSize: 12 }}>{pendingError}</span>
          )}
