...
// "ocp-cluster · " eklemek gürültü, cluster'sız seriye önek basmak yalan.
describe('withClusterPrefix / podClusterOf', () => {
  it('çok cluster + cluster bilinen → önek', () => {
    expect(withClusterPrefix('6f9c7b-x2v', 'ocp-cluster', true)).toBe('ocp-cluster · 6f9c7b-x2v');
  });
  it('tek cluster → önek YOK (gürültü)', () => {
    expect(withClusterPrefix('6f9c7b-x2v', 'ocp-cluster', false)).toBe('6f9c7b-x2v');
  });
  ...
  it('podClusterOf groupKey[3] okur, kırpar, yoksa boş', () => {
    expect(podClusterOf(['p', 'h', 'i', ' ocp-cluster '])).toBe('ocp-cluster');
  ...
  it('4. anahtar eklenmesi podLineLabel sözleşmesini BOZMAZ', () => {
    ...
    expect(podLineLabel(['auth-service-6f9c7b-x2v', '', 'uuid', 'ocp-cluster'], 'auth-service', 'used'))
      .toBe('6f9c7b-x2v');
  });
});
