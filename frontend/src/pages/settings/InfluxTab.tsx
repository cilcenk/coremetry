...
import {
  ...
  thresholdsToForm, thresholdsToWire, TFAIL_TEMPLATE, GG_TOTAL_TEMPLATE, TFAIL_RATIO_TEMPLATE,
    ...
    <div style={{ maxWidth: 820 }}>
      ...
      <form onSubmit={save}>
          ...
          return (
            <div key={i} className={`influx-src${r.enabled ? '' : ' is-off'}`}>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 10, alignItems: 'center' }}>
                ...
                {/* v0.10.532 — hata oranı: tfail_adet ÷ gg_adet_total; iki girdi yoksa devre dışı. */}
                <Button type="button" variant="secondary" size="sm"
                  disabled={r.queries.some(q => q.name === TFAIL_RATIO_TEMPLATE.name)
                    || !r.queries.some(q => q.name === TFAIL_TEMPLATE.name)
                    || !r.queries.some(q => q.name === GG_TOTAL_TEMPLATE.name)}
                  title={`${TFAIL_TEMPLATE.name} ve ${GG_TOTAL_TEMPLATE.name} sorguları gerekir`}
                  onClick={() => patch(i, { queries: [...r.queries, queryFromWire(TFAIL_RATIO_TEMPLATE)] })}>
                  + Hata oranı şablonu
                </Button>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <Button type="button" variant="secondary" size="sm"
            onClick={() => setRows(rs => [...rs, { ...EMPTY_SOURCE, queries: [queryFromWire(TFAIL_TEMPLATE), queryFromWire(GG_TOTAL_TEMPLATE), queryFromWire(TFAIL_RATIO_TEMPLATE)] }])}>
            + Kaynak ekle
          </Button>
