...
// "REDACTED şablonu" düğmesi spec'in iki sorgusunu + attrMap'i doldurur.
  ...
  return (
    <div style={{ maxWidth: 820 }}>
      ...
      <p style={{ color: 'var(--text2)', fontSize: 13, marginBottom: 16 }}>
        InfluxDB 2.x'teki bir seriyi (ör. <code>REDACTED/REDACTED</code>) dış metrik kaynağı olarak
      ...
      <form onSubmit={save}>
          ...
          return (
            <div key={i} className={`influx-src${r.enabled ? '' : ' is-off'}`}>
                ...
                <div key={k} className="influx-q">
                  <div className="influx-row">
                    <Field label="Sorgu adı (metrik: ext:<ad>)" value={q.name}
                      ...
                      placeholder="REDACTED" hint="a-z, 0-9, _" className="is-narrow" />
                  ...
                  <TextareaField label="Tag → attribute eşlemesi (satır başına TAG=attr)" rows={6} value={q.attrMap}
                    ...
                    hint="REDACTED=operation · REDACTED=error.code · REDACTED=k8s.pod.name · TRACEID=trace_id" />
                  <div className="influx-row">
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 10, alignItems: 'center' }}>
                ...
                <Button type="button" variant="secondary" size="sm"
                  disabled={r.queries.some(q => q.name === REDACTEDTEMPLATE.name)}
                  onClick={() => patch(i, { queries: [...r.queries, queryFromWire(REDACTEDTEMPLATE)] })}>
                  + REDACTED şablonu
                </Button>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <Button type="button" variant="secondary" size="sm"
            onClick={() => setRows(rs => [...rs, { ...EMPTY_SOURCE, queries: [queryFromWire(REDACTEDTEMPLATE)] }])}>
            + Kaynak ekle
          </Button>
