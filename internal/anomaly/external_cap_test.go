...
func TestExternalScan_OpenCapLimitsNewOpens(t *testing.T) {
	...
	if summary.Value != 4 || summary.Service != "ext:REDACTED" || summary.Kind != chstore.ProblemKindExternal || summary.Status != "open" {
...
func TestExternalScan_OpenCapPrefersStrongestZ(t *testing.T) {
	...
	for _, p := range f.upserts {
		...
		if p.Service != "ext:REDACTED/OPB/E1" {
...
func TestExternalScan_OpenCapDoesNotGateRefresh(t *testing.T) {
	...
	for _, sr := range series {
		subj := ExternalSubject("REDACTED", sr.GroupKey)
		open = append(open, chstore.Problem{ID: "p-" + subj, RuleID: "anomaly:" + subj + ":ext:REDACTED",
...
func TestExternalScan_OpenCapSummaryResolvesWhenClear(t *testing.T) {
	...
	now := time.Date(2026, 9, 10, 10, 0, 0, 0, time.UTC)
	summary := chstore.Problem{ID: "cap-1", RuleID: "anomaly:ext-cap:ext:REDACTED:ext:REDACTED",
		Service: "ext:REDACTED", Kind: chstore.ProblemKindExternal, Status: "open", Value: 9, StartedAt: now.Add(-time.Hour).UnixNano()}
