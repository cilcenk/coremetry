...
func TestExternalScan_ClustersSameFirstDimension(t *testing.T) {
	...
	if cluster.Service != "ext:ggfail/OP_PAY" || cluster.Kind != chstore.ProblemKindExternal || cluster.Value != 4 {
...
func TestExternalScan_SingleDimensionNeverClusters(t *testing.T) {
	...
	target := extTarget
	target.GroupBy = []string{"OPERATIONCODE"}
...
func TestExternalScan_ClusterResolvesWhenMembersCalm(t *testing.T) {
	cfg := chstore.DefaultAnomalySensitivity()
	now := time.Date(2026, 9, 10, 14, 0, 0, 0, time.UTC)
	open := chstore.Problem{ID: clusterProblemID("ext:ggfail/OP_PAY"), RuleID: clusterRulePrefix + "ext:ggfail/OP_PAY",
		Service: "ext:ggfail/OP_PAY", Kind: chstore.ProblemKindExternal, Status: "open", Value: 4, StartedAt: now.Add(-time.Hour).UnixNano()}
