...
func TestInboxListKeyIncludesSearch(t *testing.T) {
	base := inboxListKey("open", "", "", "", "", "", 200, "priority", "desc", 5)

	t.Run("search term changes the key", func(t *testing.T) {
		if got := inboxListKey("open", "", "timeout", "", "", "", 200, "priority", "desc", 5); got == base {
	...
	t.Run("different terms never collide", func(t *testing.T) {
		...
		for _, term := range []string{"", "timeout", "OOMKilled", "504", "time", "out"} {
			k := inboxListKey("open", "", term, "", "", "", 200, "priority", "desc", 5)
	...
	t.Run("search is independent of the service filter", func(t *testing.T) {
		...
		a := inboxListKey("open", "api", "", "", "", "", 200, "priority", "desc", 5)
		b := inboxListKey("open", "", "api", "", "", "", 200, "priority", "desc", 5)
		if a == b {
	...
	t.Run("every other input still separates", func(t *testing.T) {
		...
		for name, k := range map[string]string{
			"base":   base,
			"status": inboxListKey("all", "", "", "", "", "", 200, "priority", "desc", 5),
			"svc":    inboxListKey("open", "api", "", "", "", "", 200, "priority", "desc", 5),
			"owner":  inboxListKey("open", "", "", "team-a", "", "", 200, "priority", "desc", 5),
			"sre":    inboxListKey("open", "", "", "", "sre-a", "", 200, "priority", "desc", 5),
			"env":    inboxListKey("open", "", "", "", "", "prod", 200, "priority", "desc", 5),
			"limit":  inboxListKey("open", "", "", "", "", "", 50, "priority", "desc", 5),
			...
			"sortCol": inboxListKey("open", "", "", "", "", "", 200, "lastSeen", "desc", 5),
			"sortDir": inboxListKey("open", "", "", "", "", "", 200, "priority", "asc", 5),
			...
			"minOcc0":  inboxListKey("open", "", "", "", "", "", 200, "priority", "desc", 0),
			"minOcc10": inboxListKey("open", "", "", "", "", "", 200, "priority", "desc", 10),
		} {
			if prev, dup := seen[k]; dup {
