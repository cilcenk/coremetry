...
type CHCoordinator struct {
	...
	// okuma gecikmesi okunamaz hale gelir.
...
func (s *Server) getCHCoordinatorSpread(w http.ResponseWriter, r *http.Request) {
	...
	s.serveCached(w, r, key, 60*time.Second, func(ctx context.Context) (any, error) {
		...
		rows, err := s.store.Conn().Query(ctx, coordinatorQuery(cn, windowS))
		if err != nil {
			// Ölçüm yapılamaması panelin çökmesi demek değil —
			// query_log kapalı olabilir (log_queries=0) ya da
			// kullanılan kimlik system tablolarına erişemiyor.
			// Sessiz boş panel yerine nedeni söyle.
			out.Note = "query_log okunamadı: " + err.Error() +
				" — CH'de log_queries kapalı veya kimliğin system.query_log erişimi yok."
			return out, nil
		}
		defer rows.Close()
		...
		for rows.Next() {
			var c CHCoordinator
			if err := rows.Scan(&c.Host, &c.Selects, &c.Inserts, &c.Other,
