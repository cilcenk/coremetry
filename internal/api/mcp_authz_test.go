...
// koşar (bugün 19 tool'un tamamı "" olduğu için sentetik olmadan
...
// Bugünkü duruş: 19 tool'un tamamı MinRole "" (salt-okunur + REST eşi
...
func TestAllShippedToolsAreViewerLevel(t *testing.T) {
	for _, tool := range mcptools.ToolList(mcptools.Deps{}) {
		if tool.MinRole != "" {
