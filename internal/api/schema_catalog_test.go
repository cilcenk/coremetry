...
func TestSchemaEvidenceEndToEnd(t *testing.T) {
	...
	se := s.buildSchemaEvidence(testDB2Stack,
		[]string{"INSERT INTO REDACTED (MUSTERI_NO, TELNO) VALUES (?, ?)"}, nil)
	if !se.Signal || se.Columns != 2 {
		t.Fatalf("kanıt: signal=%v columns=%d\n%s", se.Signal, se.Columns, se.Block)
	}
	for _, want := range []string{"ŞEMA BAĞLAMI", "2026-08-28", "SQLCODE -302, SQLSTATE 22001", "REDACTED.TELNO VARCHAR(10) NOT NULL", "REDACTED.MUSTERI_NO DECIMAL(15) NOT NULL"} {
	...
	if !strings.Contains(sent, "REDACTED.TELNO VARCHAR(10) NOT NULL") {
	...
	if !strings.Contains(sample, "[şema: REDACTED · 2 kolon]") || !strings.Contains(sample, "[kod: core-service/src/CardService.java:27-31") {
	...
	if p := fp2.sent()[0]; !strings.Contains(p, "KOD BAĞLAMI İSTENDİ — ÇÖZÜLEMEDİ") || !strings.Contains(p, "REDACTED.TELNO VARCHAR(10)") {
...
func TestSchemaEvidenceDegradesHonestly(t *testing.T) {
	...
	se = s.buildSchemaEvidence("java.sql.SQLException: timeout", nil,
		[]string{"11| <select id=\"q\">\n12| SELECT TELNO FROM REDACTED WHERE MUSTERI_NO = #{m}\n13| </select>"})
