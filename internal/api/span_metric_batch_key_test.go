...
func baseKey() string {
	return spanMetricBatchKey(tFrom, tTo, 60, []string{"service.name"}, "", `service.name = "a"`,
...
func TestSpanMetricBatchKey_EveryInputSeparates(t *testing.T) {
	...
	cases := map[string]string{
		"from": spanMetricBatchKey(tFrom+int64(time.Hour), tTo, 60, []string{"service.name"}, "", `service.name = "a"`,
			[]chstore.SpanMetricAggSpec{agg("rate", "rate", ""), agg("p99", "p99", "duration_ms")}),
		"to": spanMetricBatchKey(tFrom, tTo+int64(time.Hour), 60, []string{"service.name"}, "", `service.name = "a"`,
			[]chstore.SpanMetricAggSpec{agg("rate", "rate", ""), agg("p99", "p99", "duration_ms")}),
		"step": spanMetricBatchKey(tFrom, tTo, 300, []string{"service.name"}, "", `service.name = "a"`,
			[]chstore.SpanMetricAggSpec{agg("rate", "rate", ""), agg("p99", "p99", "duration_ms")}),
		"groupBy": spanMetricBatchKey(tFrom, tTo, 60, []string{"host.name"}, "", `service.name = "a"`,
			[]chstore.SpanMetricAggSpec{agg("rate", "rate", ""), agg("p99", "p99", "duration_ms")}),
		"filters": spanMetricBatchKey(tFrom, tTo, 60, []string{"service.name"}, `[{"k":"x"}]`, `service.name = "a"`,
			[]chstore.SpanMetricAggSpec{agg("rate", "rate", ""), agg("p99", "p99", "duration_ms")}),
		"dsl": spanMetricBatchKey(tFrom, tTo, 60, []string{"service.name"}, "", `service.name = "b"`,
			[]chstore.SpanMetricAggSpec{agg("rate", "rate", ""), agg("p99", "p99", "duration_ms")}),
		"agg field": spanMetricBatchKey(tFrom, tTo, 60, []string{"service.name"}, "", `service.name = "a"`,
			[]chstore.SpanMetricAggSpec{agg("rate", "rate", ""), agg("p99", "p99", "latency_ms")}),
		"agg kind": spanMetricBatchKey(tFrom, tTo, 60, []string{"service.name"}, "", `service.name = "a"`,
			[]chstore.SpanMetricAggSpec{agg("rate", "rate", ""), agg("p99", "p95", "duration_ms")}),
		"extra agg": spanMetricBatchKey(tFrom, tTo, 60, []string{"service.name"}, "", `service.name = "a"`,
...
func TestSpanMetricBatchKey_NoBoundaryCollision(t *testing.T) {
	a := spanMetricBatchKey(tFrom, tTo, 60, []string{"a", "bc"}, "", "", nil)
	b := spanMetricBatchKey(tFrom, tTo, 60, []string{"ab", "c"}, "", "", nil)
	if a == b {
		t.Fatal(`group-by ["a","bc"] and ["ab","c"] collided — separator missing`)
	}
	c := spanMetricBatchKey(tFrom, tTo, 60, nil, "x", "y", nil)
	d := spanMetricBatchKey(tFrom, tTo, 60, nil, "xy", "", nil)
	if c == d {
		t.Fatal("filters/dsl boundary collided — separator missing")
	}
	e := spanMetricBatchKey(tFrom, tTo, 60, nil, "", "", []chstore.SpanMetricAggSpec{agg("n", "a", "f")})
	f := spanMetricBatchKey(tFrom, tTo, 60, nil, "", "", []chstore.SpanMetricAggSpec{agg("n", "af", "")})
	if e == f {
...
func TestSpanMetricBatchKey_OrderInsensitive(t *testing.T) {
	a := spanMetricBatchKey(tFrom, tTo, 60, []string{"a", "b"}, "", "",
		[]chstore.SpanMetricAggSpec{agg("rate", "rate", ""), agg("p99", "p99", "duration_ms")})
	b := spanMetricBatchKey(tFrom, tTo, 60, []string{"b", "a"}, "", "",
...
func TestSpanMetricBatchKey_WindowBucketedToMinute(t *testing.T) {
	base := spanMetricBatchKey(tFrom, tTo, 60, nil, "", "", nil)
	within := spanMetricBatchKey(tFrom+int64(37*time.Second), tTo+int64(11*time.Second), 60, nil, "", "", nil)
	if base != within {
		t.Fatal("sub-minute jitter must land on the same key, otherwise nothing ever caches")
	}
	next := spanMetricBatchKey(tFrom+int64(90*time.Second), tTo, 60, nil, "", "", nil)
	if base == next {
