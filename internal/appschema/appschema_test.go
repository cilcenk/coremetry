...
SHOP,REDACTED,MUSTERI_NO,DECIMAL,15,0,N
SHOP,REDACTED,TELNO,VARCHAR,10,0,N
SHOP,REDACTED,ACIKLAMA,VARCHAR,200,0,Y
SHOP,MUSTERI,MUSTERI_NO,DECIMAL,15,0,N
`
...
func TestParseCSVDialects(t *testing.T) {
	cases := []struct {
		...
		{"el yazımı", "table,column,type,length,nullable\nREDACTED,MUSTERI_NO,DECIMAL,15,N\nREDACTED,TELNO,VARCHAR,10,N\nREDACTED,ACIKLAMA,VARCHAR,200,Y\nMUSTERI,MUSTERI_NO,DECIMAL,15,N\n"},
	}
	for _, c := range cases {
...
func TestSQLErrorSignalTable(t *testing.T) {
	cases := []struct {
		...
		{"java.sql.SQLException: ORA-12899: value too large for column \"SHOP\".\"REDACTED\".\"TELNO\" (actual: 11, maximum: 10)", "oracle", "ORA-12899", "", "uzunluğunu aşıyor", true},
...
func TestTargetsOfTable(t *testing.T) {
	cases := []struct {
		...
		{"insert kolon listesi", "INSERT INTO REDACTED (MUSTERI_NO, TELNO, ACIKLAMA) VALUES (?, ?, ?)", Targets{Tables: []string{"REDACTED"}, Columns: []string{"MUSTERI_NO", "TELNO", "ACIKLAMA"}}},
		{"update set", "update int_tfraud set telno = #{telNo}, aciklama=#{a} where musteri_no = ?", Targets{Tables: []string{"REDACTED"}, Columns: []string{"TELNO", "ACIKLAMA"}}},
		{"select join", "SELECT t.TELNO FROM REDACTED t JOIN MUSTERI m ON m.MUSTERI_NO = t.MUSTERI_NO WHERE t.TELNO = ?", Targets{Tables: []string{"REDACTED", "MUSTERI"}}},
		{"delete", "DELETE FROM REDACTED WHERE MUSTERI_NO = ?", Targets{Tables: []string{"REDACTED"}}},
		{"mapper bloğu satır numaralı + XML + CDATA", "11| <select id=\"ariCTelefonSelect\">\n12|   SELECT <include refid=\"cols\"/>\n13|     FROM REDACTED\n14|    WHERE TELNO = <![CDATA[ #{telNo} ]]>\n15| </select>", Targets{Tables: []string{"REDACTED"}}},
		{"boş", "   ", Targets{}},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
...
func TestLookupAndPromptSection(t *testing.T) {
	...
	cols, dropped := Lookup(cat, TargetsOf("INSERT INTO REDACTED (MUSTERI_NO, TELNO) VALUES (?, ?)"))
	...
	cols, _ = Lookup(cat, TargetsOf("INSERT INTO REDACTED (MUSTERI_NO, TELNO, ACIKLAMA) VALUES (?, ?, ?)"))
	...
	for _, want := range []string{"ŞEMA BAĞLAMI", "2026-08-28", "SQLCODE -302, SQLSTATE 22001 — değer kolon uzunluğunu aşıyor",
		"REDACTED.TELNO VARCHAR(10) NOT NULL", "REDACTED.MUSTERI_NO DECIMAL(15) NOT NULL", "REDACTED.ACIKLAMA VARCHAR(200) NULL"} {
