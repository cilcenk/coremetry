...
func TestPickSearchHitRecency_NewestRepoWins(t *testing.T) {
	...
	hits := []CodeSearchHit{
		{Project: "SHOP", Repository: "card-legacy", Path: "/src/com/example/card/CardDetailBusiness.java", Branch: "release"},
		{Project: "SHOP", Repository: "card-v2", Path: "/other/CardDetailBusiness.java", Branch: "master"},
		{Project: "SHOP", Repository: "card-v2", Path: "/other/CardDetailBusiness.java", Branch: "release"},
	}
	rec := map[string]time.Time{
		RecencyKey("SHOP", "card-legacy"): time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC),
		RecencyKey("SHOP", "card-v2"):     time.Date(2026, 8, 30, 0, 0, 0, 0, time.UTC),
	}
	...
	h, ok = PickSearchHitRecency(hits, "", frame, nil, map[string]time.Time{RecencyKey("SHOP", "card-v2"): time.Now()})
