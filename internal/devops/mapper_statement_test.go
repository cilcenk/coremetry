...
<mapper namespace="com.acme.fraud.IntTfraudMapper">
  ...
  <insert id="ariCTelefonInsert">
    INSERT INTO REDACTED (MUSTERI_NO, TELNO) VALUES (#{musteriNo}, #{telNo})
  </insert>
</mapper>
`
...
func TestMapperStatementWindowTable(t *testing.T) {
	cases := []struct {
		...
		{"select bloğu, gerçek satır numaralarıyla", "ariCTelefonSelect", 11, 16, "FROM REDACTED", false},
		{"resultMap aynı id'yi taşıyor ama STATEMENT önce", "ariCTelefonSelect", 11, 16, "<select id=\"ariCTelefonSelect\"", false},
		{"insert bloğu", "ariCTelefonInsert", 17, 19, "INSERT INTO REDACTED", false},
...
func TestHuntResourcesPrefersStatementBlock(t *testing.T) {
	...
	if ws[0].FromLine != 11 || !strings.Contains(ws[0].Content, "FROM REDACTED") || ws[0].Frame != "statement id: ariCTelefonSelect" {
