...
func TestMatchRepoName(t *testing.T) {
	...
	// (shop-cashmanagement-cashflow-prod → cashmanagement-cashflow),
...
func TestMatchRepoNameNear(t *testing.T) {
	...
	t.Run("içerme önek uzunluğunu yener", func(t *testing.T) {
		// "cashflow" isteniyor: "shop-cashflow" ÖNEKten hiç puan almaz
		// (ortak önek 0) ama içerdiği için en üste çıkmalı.
		m := MatchRepoName("cashflow", []string{"cashier-desk", "shop-cashflow"})
		if len(m.Near) == 0 || m.Near[0] != "shop-cashflow" {
			t.Fatalf("Near=%v, ilk sırada shop-cashflow beklenirdi", m.Near)
		}
	})
}
...
func TestReposURLShape(t *testing.T) {
	cfg := Settings{BaseURL: "https://tfs.example/tfs/", Collection: "DefaultCollection", Project: "SHOP"}
	root := reposURL(cfg)
	if root != "https://tfs.example/tfs/DefaultCollection/SHOP/_apis/git/repositories" {
