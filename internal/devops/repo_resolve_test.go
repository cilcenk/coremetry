...
func TestResolveRepo(t *testing.T) {
	tests := []struct {
		...
		{name: "-int eki", service: "shop-core-int", repo: "core", source: RepoSourceConvention},
		{name: "-prep eki", service: "shop-core-prep", repo: "core", source: RepoSourceConvention},
		{
			// Tek ek soyulur: -prod-prod gerçek bir konvansiyon değil,
			// döngüsel soyma sürprizi olurdu.
			name:    "yalnız SON ortam eki soyulur",
			service: "shop-core-uat-prod",
			repo:    "core-uat", source: RepoSourceConvention,
		},
		{
			name:    "ad tamamen önekten ibaret → soyma yok",
			service: "shop-",
			repo:    "shop-", source: RepoSourceConvention,
		},
		{
		...
		},
		{
			...
			cfg:     ResolveConfig{RepoPrefixes: []string{"shop-", "svc-"}},
			repo:    "orders", source: RepoSourceConvention,
		},
		// --- pin kazanır ---
		{
			name:    "pin düz ad",
			service: "shop-digital-mobile-pushconfirm-prod", meta: "pushconfirm-legacy",
			repo: "pushconfirm-legacy", source: RepoSourcePin,
		},
		{
			name:    "pin _git URL'si",
			service: "shop-core-prod",
		...
		},
		{
			...
			service: "shop-core-prod", meta: "git@devops.example.local:Proj/core-service.git",
			repo: "core-service", source: RepoSourcePin,
		},
		{
			name:    "pin sondaki eğik çizgi",
			service: "shop-core-prod", meta: "https://devops.example.local/Coll/Proj/_git/core-service/",
			repo: "core-service", source: RepoSourcePin,
		},
		{
			// Pin, konvansiyonu EZER: soyma uygulanmaz. Operatör
			// deposunu "shop-" ile adlandırdıysa öyle kalır.
			name:    "pin konvansiyona uğramaz",
			service: "shop-core-prod", meta: "shop-core",
			repo: "shop-core", source: RepoSourcePin,
		},
		{
			name:    "yalnız boşluktan ibaret pin → konvansiyon",
			service: "shop-core-prod", meta: "   ",
...
//
// Operatör isteği: "service_name başında shop- yazıyorsa direkt project SHOP
// olduğunu anlasın."
//
...
func TestResolveRepoProjectFromPrefix(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			"ops-gateway-prod", []string{"shop-", "ops-"},
			"gateway", "OPS",
		},
	}
	for _, c := range cases {
...
// türetilen projede aratmak (burada OTHER yerine SHOP) sunucuda sessiz bir
// 404 üretir. Korunan davranış aynen duruyor — Project ASLA "SHOP"
...
func TestResolveRepoPinProjectWinsOverPrefix(t *testing.T) {
	got := ResolveRepo("shop-payments-core-prod", "https://tfs.example.com/DefaultCollection/OTHER/_git/payments-core", ResolveConfig{})
	if got.Source != RepoSourcePin {
		t.Fatalf("Source=%q, pin bekleniyordu", got.Source)
	}
	if got.Project.Value == "SHOP" {
		t.Fatalf("pinin projesi (OTHER) yerine servis önekinden türetilen SHOP " +
...
func TestResolveRepoPinWithoutProjectDerives(t *testing.T) {
	...
	if got.Project.Value != "SHOP" || got.Project.Source != RepoSourceConvention {
		t.Fatalf("Project=%+v, istenen {SHOP convention} — pin projeyi söylemiyorsa "+
...
func TestResolveRepoProjectMissReason(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			[]string{"katalog pini yalnız depo adı taşıyor", "standalone-service-prod", "shop-"},
		},
		{
			"pinsiz, önek tutmuyor",
			"standalone-service-prod", "",
			[]string{"katalogda depo pini yok", "standalone-service-prod", "shop-"},
		},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
...
func TestParsePinnedRepo(t *testing.T) {
	cases := []struct {
		...
		{"düz ad — boşluklu", "  shop-core  ", "", "shop-core"},
		// (2) proje/depo
		{"proje/depo", "SHOP/payments-core", "SHOP", "payments-core"},
		{"proje/depo — .git eki", "SHOP/payments-core.git", "SHOP", "payments-core"},
		{"üç parça, _git yok → proje TAHMİN EDİLMEZ", "Coll/SHOP/payments-core", "", "payments-core"},
		// (3) tam URL
		{
