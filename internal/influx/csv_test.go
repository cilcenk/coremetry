...
#default,_result,,,,,,,
,result,table,_start,_stop,_time,_value,REDACTED,REDACTED
...
func TestParseAnnotatedCSV_TwoTables(t *testing.T) {
	...
	if recs[0].Values["REDACTED"] != "OP1" || recs[0].Values["_value"] != "42" {
		t.Fatalf("record 0 wrong: %+v", recs[0].Values)
	}
	if recs[2].Table != 1 || recs[2].Values["REDACTED"] != "E1" || recs[2].Values["REDACTED"] != "OP2" {
