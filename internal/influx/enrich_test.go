...
func enrichQuery() QueryConfig {
	return QueryConfig{
		Name:       "REDACTED",
		GroupBy:    []string{"REDACTED", "REDACTED"},
		AttrMap:    map[string]string{"REDACTED": "operation_code"},
		REDACTED") |> range(start: {{from}}, stop: {{to}}) |> filter(fn: (r) => r.REDACTED == "{{op}}" and r.REDACTED == "{{err}}") ENRICH-MARK`,
	}
}
...
func baseReq(now time.Time) EnrichRequest {
	return EnrichRequest{
		ProblemID: "p1", Subject: "ext:REDACTED/OP1/E1",
		Source: SourceConfig{ID: "i-1", Name: "REDACTED"}, Query: enrichQuery(),
...
func TestEnrich_HappyPath(t *testing.T) {
	...
	api := &fakeQueryAPI{recs: map[string][]Record{"ENRICH-MARK": recs(
		REDACTED"},
		map[string]string{"_time": "2026-09-02T09:59:00Z", "TRACEID": validB, "REDACTED": "pod-b"},
		map[string]string{"_time": "2026-09-02T09:59:30Z", "TRACEID": validB, "REDACTED": "pod-b"},
		map[string]string{"_time": "2026-09-02T09:57:00Z", "TRACEID": "not-a-trace", "REDACTED": "pod-c"},
	)}}
	...
	if !strings.Contains(api.lastFlux, `r.REDACTED == "OP1" and r.REDACTED == "E1"`) || !strings.Contains(api.lastFlux, "range(start: 2026-09-02T09:50:00Z, stop: 2026-09-02T10:00:00Z)") {
		t.Fatalf("template: %s", api.lastFlux)
	}
	// exemplar: normalize edilmiş küçük-harf id, poller ile AYNI fingerprint
	ex := store.exemplars[0]
	if ex.TraceID != validB || ex.Metric != "ext:REDACTED" || ex.Service != "REDACTED" || ex.Value != 1 {
		t.Fatalf("exemplar newest-first/normalized: %+v", ex)
	}
	wantFP := otlp.SeriesFingerprint("ext:REDACTED", []*commonpb.KeyValue{strKV("operation_code", "OP1"), strKV("REDACTED", "E1")}, "REDACTED", "")
	...
	h := store.hyp
	if h == nil || h.AnchorKind != "problem" || h.AnchorID != "p1" || h.Service != "ext:REDACTED/OP1/E1" || h.Deep == nil {
...
func TestEnrichVars(t *testing.T) {
	...
	v := enrichVars([]string{"REDACTED", "REDACTED", "REDACTED"}, []string{"MOBIL", "OP1", "E1"}, from, to)
	if v["op"] != "OP1" || v["err"] != "E1" || v["REDACTED"] != "MOBIL" || v["REDACTED"] != "OP1" {
		t.Fatalf("vars: %v", v)
	}
