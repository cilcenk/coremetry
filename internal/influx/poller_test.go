...
func TestWorkerTick_RatioQueryDerivesPercent(t *testing.T) {
	...
	src.Queries = []QueryConfig{
		{Name: "tfail_adet", Flux: `from(bucket: "GGFailTraceBckt")`, GroupBy: []string{"OPERATIONCODE"}, AttrMap: map[string]string{"OPERATIONCODE": "operation"}},
		{Name: "gg_adet_total", Flux: `from(bucket: "GoldenGateBucket")`, GroupBy: []string{"OPERATIONCODE"}, AttrMap: map[string]string{"OPERATIONCODE": "operation"}},
		{Name: "tfail_oran", GroupBy: []string{"OPERATIONCODE"}, AttrMap: map[string]string{"OPERATIONCODE": "operation"},
			Ratio: &RatioSpec{Numerator: "tfail_adet", Denominator: "gg_adet_total", MinDenominator: 10}},
	}
	svc.Configure(Settings{Sources: []SourceConfig{src}})
	now := time.Date(2026, 9, 7, 9, 45, 0, 0, time.UTC)
	bt := "2026-09-07T09:41:00Z"
	q := &fakeQueryAPI{recs: map[string][]Record{
		"GGFailTraceBckt": recs(map[string]string{"_value": "5", "OPERATIONCODE": "OP1", "_time": bt}),
		"GoldenGateBucket": recs(
			map[string]string{"_value": "50", "OPERATIONCODE": "OP1", "_time": bt},
			map[string]string{"_value": "100", "OPERATIONCODE": "OP2", "_time": bt},
			map[string]string{"_value": "4", "OPERATIONCODE": "OP3", "_time": bt}),
	}}
	sink := &fakeSink{}
	w := NewWorker(svc, sink)
	w.now = func() time.Time { return now }
	...
	if strings.Join(hooked, ",") != "tfail_adet,gg_adet_total,tfail_oran" {
...
func TestWorkerTick_RatioSkippedWhenInputFails(t *testing.T) {
	...
	src := tfailSource()
	src.Queries = []QueryConfig{
		{Name: "tfail_adet", Flux: `from(bucket: "GGFailTraceBckt")`, GroupBy: []string{"OPERATIONCODE"}},
		{Name: "gg_adet_total", Flux: `from(bucket: "GoldenGateBucket")`, GroupBy: []string{"OPERATIONCODE"}},
		{Name: "tfail_oran", GroupBy: []string{"OPERATIONCODE"}, Ratio: &RatioSpec{Numerator: "tfail_adet", Denominator: "gg_adet_total"}},
	}
	svc.Configure(Settings{Sources: []SourceConfig{src}})
	now := time.Date(2026, 9, 7, 9, 45, 0, 0, time.UTC)
