...
func TestBuildMetricsRequest_Shape(t *testing.T) {
	...
	req, drops := BuildMetricsRequest(src, qc, recs(
		map[string]string{"_value": "42", "REDACTED": "OP1", "REDACTED": "E1", "_time": "2026-09-01T10:00:00Z"},
		map[string]string{"_value": "7", "REDACTED": "OP1", "REDACTED": "E2"},
	), now)
	if drops.Total() != 0 {
	...
	if res["service.name"] != "REDACTED" || res["coremetry.source"] != "influx" || res["influx.source.id"] != "i-aaaaaaaa" {
	...
	if len(ms) != 1 || ms[0].Name != "ext:REDACTED" {
...
func TestBuildMetricsRequest_Drops(t *testing.T) {
	...
	qc.GroupBy = []string{"REDACTED", "REDACTED", "REDACTED"}
	now := time.Now()
	req, drops := BuildMetricsRequest(src, qc, recs(
		REDACTED"},
		REDACTED
	...
	if g.DataPoints[0].Attributes[2].Key != "REDACTED" {
...
func TestBuildMetricsRequest_Cap(t *testing.T) {
	...
	for i := 0; i < MaxRowsPerQuery+5; i++ {
		rows = append(rows, Record{Values: map[string]string{"_value": "1", "REDACTED": "OP", "REDACTED": "E"}})
	}
...
func TestWorkerTick_PollsDueSourcesAndInserts(t *testing.T) {
	...
	REDACTED(
		map[string]string{"_value": "42", "REDACTED": "OP1", "REDACTED": "E1"},
		map[string]string{"_value": "7", "REDACTED": "OP2", "REDACTED": "E1"},
	)}}
	sink := &fakeSink{}
	...
	p := sink.pts[0]
	if p.Metric != "ext:REDACTED" || p.ServiceName != "REDACTED" || p.Instrument != "gauge" || p.Value != 42 {
...
func TestBuildMetricsRequest_UsesBucketTime(t *testing.T) {
	...
	now := time.Date(2026, 9, 1, 21, 32, 30, 0, time.UTC)
	req, drops := BuildMetricsRequest(src, qc, recs(
		map[string]string{"_time": "2026-09-01T21:32:00Z", "_value": "16", "REDACTED": "A", "REDACTED": "E"},
		map[string]string{"_value": "9", "REDACTED": "B", "REDACTED": "E"},
	), now)
	if drops.Total() != 0 {
...
func TestWorkerTick_WatermarkAcrossPolls(t *testing.T) {
	...
	now := time.Date(2026, 9, 1, 21, 32, 30, 0, time.UTC)
	REDACTED(
		map[string]string{"_time": "2026-09-01T21:31:00Z", "_value": "7", "REDACTED": "A", "REDACTED": "E"},
		map[string]string{"_time": "2026-09-01T21:32:00Z", "_value": "16", "REDACTED": "A", "REDACTED": "E"},
		map[string]string{"_time": "2026-09-01T21:33:00Z", "_value": "2", "REDACTED": "A", "REDACTED": "E"}, // kısmi
	)}}
	w := NewWorker(svc, sink)
	w.now = func() time.Time { return now }
	w.queryAPIFor = func(SourceConfig) (QueryAPI, error) { return q, nil }
...
func TestQueryConfigMetricGroupBy_AppliesAttrMap(t *testing.T) {
	qc := QueryConfig{
		...
		AttrMap: map[string]string{"REDACTED": "operation_code", "REDACTED": ""},
	}
	got := qc.MetricGroupBy()
	if len(got) != 2 || got[0] != "operation_code" || got[1] != "REDACTED" {
