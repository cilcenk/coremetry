...
func validSource() SourceConfig {
	return SourceConfig{
		Name: "REDACTED", URL: "https://influx.example:8086", Org: "bank",
		TokenRef: "env:COREMETRY_INFLUX_TOKEN_GG", Enabled: true,
		Queries: []QueryConfig{{
			Name:    "REDACTED",
			REDACTED") |> range(start: -2m) |> sum()`,
			GroupBy: []string{"REDACTED", "REDACTED"},
			AttrMap: map[string]string{"REDACTED": "operation", "REDACTED": "error.code"},
		}},
	}
}
...
func TestNormalize_DefaultsAndIDs(t *testing.T) {
	...
	renamed.Sources[0].Name = "REDACTED-prod"
...
func TestNormalize_Rejects(t *testing.T) {
	...
	cases := []struct {
		...
		{"query name not a slug", mut(func(s *SourceConfig) { s.Queries[0].Name = "REDACTED adet" }), "sorgu adı"},
...
func TestNormalize_Ratio(t *testing.T) {
	base := func() SourceConfig {
		...
		s.Queries = append(s.Queries, QueryConfig{
			REDACTED") |> range(start: -2h)`,
			GroupBy: []string{"REDACTED", "REDACTED"},
		}, QueryConfig{
			Name: "tfail_oran", GroupBy: []string{"REDACTED", "REDACTED"},
			Ratio: &RatioSpec{Numerator: " REDACTED ", Denominator: "gg_adet_total"},
		})
		return s
	}
	...
	if r == nil || r.Numerator != "REDACTED" || r.Denominator != "gg_adet_total" {
	...
	// v0.10.548 — pay fazla boyut taşıyabilir (REDACTED üç boyutlu, toplam iki):
	// oran paydanın taneciğinde, fazla boyut birleşimde toplanır.
	sup := base()
	sup.Queries[0].GroupBy = []string{"REDACTED", "REDACTED", "REDACTED"}
	...
	cases := []struct {
		...
		{"aynı sorgu", func(s *SourceConfig) { s.Queries[2].Ratio.Denominator = "REDACTED" }, "aynı sorgu olamaz"},
		{"boş ad", func(s *SourceConfig) { s.Queries[2].Ratio.Denominator = "" }, "zorunlu"},
		{"groupBy farklı", func(s *SourceConfig) { s.Queries[2].GroupBy = []string{"REDACTED", "REDACTED"} }, "groupBy"},
		// v0.10.548 — pay oranın anahtarını taşımıyorsa red; payda birebir şart.
		{"pay anahtar eksik", func(s *SourceConfig) { s.Queries[0].GroupBy = []string{"REDACTED"} }, "pay sorgusunun groupBy"},
		{"REDACTED"},
