...
func TestJMXTrendQuery(t *testing.T) {
	cases := []struct {
		...
		{"heap byPod", jmxTrendQuery("shop-login", "heap", true),
			[]string{`sum by (host_name) (jvm_memory_bytes_used{area="heap",`, `job=~"^shop-login$"`}},
		{"nonheap total", jmxTrendQuery("shop-login", "nonheap", false),
			[]string{`sum(jvm_memory_bytes_used{area="nonheap",`, `job=~"^shop-login$"`}},
