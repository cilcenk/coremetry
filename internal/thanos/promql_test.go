...
func TestJMXTrendQuery(t *testing.T) {
	cases := []struct {
		...
		{"heap byPod", jmxTrendQuery("prod", "shop-login", "heap", true),
			[]string{`sum by (instance) (jvm_memory_bytes_used{area="heap",`, `namespace="prod"`, `instance=~"shop-login-.*"`}},
		{"nonheap total", jmxTrendQuery("prod", "shop-login", "nonheap", false),
			[]string{`sum(jvm_memory_bytes_used{area="nonheap",`, `instance=~"shop-login-.*"`}},
